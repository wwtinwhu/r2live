...
void img_callback1(const sensor_msgs::CompressedImageConstPtr &img_msg)
{
    ...
    header.stamp.fromSec(img_msg->header.stamp.toSec() - image_timestamp_bias);
    cv::Mat show_img = cv::imdecode(cv::Mat(img_msg->data),0);
...
//     cv::waitKey(1);
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if (i != 1 || !STEREO_TRACK)
            trackerData[i].readImage(show_img.rowRange(ROW * i, ROW * (i + 1)), img_msg->header.stamp.toSec());
        else
        {
            if (EQUALIZE)
            {
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (SHOW_TRACK)
        {
            ...
            for (int i = 0; i < NUM_OF_CAM; i++)
            {
                cv::Mat tmp_img = img.rowRange(i * ROW, (i + 1) * ROW);
                cv::cvtColor(show_img, tmp_img, CV_GRAY2RGB);

                for (unsigned int j = 0; j < trackerData[i].cur_pts.size(); j++)
                {
                    double len = std::min(1.0, 1.0 * trackerData[i].track_cnt[j] / WINDOW_SIZE);
                    // red->blue track_cnt get bigger
                    cv::circle(tmp_img, trackerData[i].cur_pts[j], 3, cv::Scalar(255 * (1 - len), 0, 255 * len), 2);
                    //draw speed line
                    /*
...
//            cv::waitKey(5);
            // Todo Why draw circle on tmp_img can influence img
            sensor_msgs::ImagePtr feature_image_msg = cv_bridge::CvImage(header,"bgr8",img).toImageMsg();
            pub_match.publish(feature_image_msg);
        }
    }
...
//     ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Debug);
    ...
    if(IMAGE_COMPRESSED)
    {
        sub_img = n.subscribe(IMAGE_TOPIC, 100, img_callback1);
    }
    else{
